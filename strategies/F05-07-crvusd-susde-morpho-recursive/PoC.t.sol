...
///                   -> Morpho collateral -> borrow USDC -> loop.
///
/// PnL one-liner:
...
///         - USDC_morpho_borrow_rate * USDC_debt
///         - 3 * curve_swap_fee (~12 bp round trip)
///
/// At block 20_650_000 sUSDe APR was ~10%, Morpho USDC borrow ~7%, crvUSD
...
contract F05_07_PoC is StrategyBase {
    ...
    function test_crvusd_susde_recursive() public {
        ...
        console2.log("Morpho USDC debt (1e6):", borrowAssetsUsdc);
    ...
    function _seedAndLoop(uint256 usdcSeed) internal {
        ...
        uint256 minOut = (ICurveStableSwap(CURVE_USDE_USDC).get_dy(int128(1), int128(0), usdcSeed) * (10_000 - SWAP_SLIPPAGE_BPS)) / 10_000;
        uint256 usdeOut = ICurveStableSwap(CURVE_USDE_USDC).exchange(int128(1), int128(0), usdcSeed, minOut);
        ...
        // Loop: borrow USDC against sUSDe, convert to USDe -> sUSDe -> redeposit.
        for (uint256 i = 0; i < LOOPS; i++) {
            uint256 borrowable = _morphoBorrowable();
            if (borrowable < 1e6) break;
            ...
            uint256 expected = ICurveStableSwap(CURVE_USDE_USDC).get_dy(int128(1), int128(0), borrowAmt);
            uint256 minOutLoop = (expected * (10_000 - SWAP_SLIPPAGE_BPS)) / 10_000;
            uint256 usdeLoopOut = ICurveStableSwap(CURVE_USDE_USDC).exchange(int128(1), int128(0), borrowAmt, minOutLoop);
            ...
            console2.log("loop USDC borrowed:", borrowAmt);
            console2.log("loop sUSDe added:", newShares);
        }
    }

    /// @dev Compute headroom USDC borrowable against the current sUSDe collateral.
    function _morphoBorrowable() internal view returns (uint256) {
        bytes32 id = _marketId(_market);
        IMorpho.Position memory pos = IMorpho(Mainnet.MORPHO).position(id, address(this));
        IMorpho.Market memory mkt = IMorpho(Mainnet.MORPHO).market(id);
        uint256 collateralUsde = ISUSDe(Mainnet.SUSDE).convertToAssets(pos.collateral);
        // USDe (18 dec) -> USDC (6 dec), assume both ~$1.

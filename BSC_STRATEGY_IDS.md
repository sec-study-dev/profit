...
| --- | ---------------------------- | -------------------------------------------------------------------------- | ----------------- | -------- |
| B01 | BNB LST 杠杆循环              | slisBNB/BNBx/stkBNB/asBNB 在 Venus/Lista lending 上的递归借贷             | wave2-B01-agent   | done     |
| B02 | BNB LST peg & basis 套利      | LST 内部 exchangeRate vs PCS/Thena/Wombat 现价的原子套利                  | wave2-B02-agent   | done     |
| B03 | Lista lisUSD CDP 机制套利     | lisUSD 软清算、redemption arb、cross-CDP basis on BSC                     | wave2-B03-agent   | in-progress |
| B04 | Pendle PT/YT on BSC          | BSC 上 PT-USDe/PT-slisBNB/PT-asBNB cash-and-carry + YT 投机              | wave2-B04-agent   | in-progress |
| B05 | Ethena USDe/sUSDe BSC carry  | USDe BSC 上的 funding 套利、sUSDe + Venus/Lista loop                     | wave2-B05-agent   | in-progress |
| B06 | Venus isolated pool 套利     | Venus V4 isolated pools 之间的 IRM 差异 + 跨池套利                       | wave2-B06-agent   | pending  |
| B07 | PCS v3 flash + cross-DEX     | PCS v3 单池 flash + Thena/Biswap/Wombat 跨 DEX peg arb                    | wave2-B07-agent   | in-progress |
| B08 | Thena/PCS ve(3,3) gauge      | veTHE/cake gauge vote、HiddenHand BSC、cross-protocol bribe basket        | wave2-B08-agent   | in-progress |
| B09 | Wombat StableSwap dynamic    | Wombat dynamic asset weight 与 PCS StableSwap/Curve 的价差                | wave2-B09-agent   | in-progress |
| B10 | 跨稳定币 CDP basis            | lisUSD × FDUSD × USDe × USD1 跨稳定币 CDP/borrow/peg basis                | wave2-B10-agent   | in-progress |
| B11 | Astherus asBNB restake stack | asBNB 在 Venus/Lista 抵押 + 底层 restake 的 stacked alpha                | wave2-B11-agent   | in-progress |
| B12 | Avalon BTC-LSD 借贷          | solvBTC/pumpBTC 等 BTC-LSD 在 Avalon 上的循环借贷                        | wave2-B12-agent   | in-progress |
...
| B15 | 三协议机制堆叠                | 必须组合 ≥3 个 BSC 协议机制的 atomic 或 positional 策略                   | wave2-B15-agent   | in-progress |
